...
with st.sidebar:
    ...
    st.caption("Steuern: in dieser Version nicht modelliert (brutto-Ansatz).")
...
with st.spinner(f"Berechne {n_possible} Rolling-Szenarien…"):
    rolling = run_rolling_backtest(returns, inflation, params)
    summary = summary_metrics(rolling)
    perc = equity_percentiles(rolling, quantiles=(0.10, 0.25, 0.50, 0.75, 0.90))
...
m1.metric("Erfolgsquote",
          f"{summary['erfolgsquote']*100:.1f} %",
          help="% der Szenarien, in denen Geld bis zum Endalter reichte")
m2.metric("Endvermögen (Median)",
          f"{summary['endvermoegen_p50']:,.0f} €")
m3.metric("CAGR (Median, money-weighted)",
          f"{summary['cagr_median']*100:.2f} %")
m4.metric("Volatilität (Median, p.a.)",
          f"{summary['volatilitaet_median']*100:.2f} %")

m5, m6, m7, m8 = st.columns(4)
m5.metric("Endvermögen P10", f"{summary['endvermoegen_p10']:,.0f} €")
m6.metric("Endvermögen P90", f"{summary['endvermoegen_p90']:,.0f} €")
m7.metric("Max Drawdown (Median)", f"{summary['max_drawdown_median']*100:.1f} %")
m8.metric("Max Drawdown (worst)", f"{summary['max_drawdown_worst']*100:.1f} %")


# ──────────────────────────────────────────────────────────────────────
# EQUITY-FAN-CHART
# ──────────────────────────────────────────────────────────────────────
st.subheader("📈 Equitykurve – alle Rolling-Szenarien (Fan-Chart)")
...
# Median
fig.add_trace(go.Scatter(
    x=x_years, y=perc["P50"], name="Median (P50)",
